/* ══════════════════════════════════════════════════════════
   SNTM SAP — script.js
   Animations haut de gamme · Performance first
   Techniques : RAF · will-change · IntersectionObserver
                FLIP · CSS custom props · passive listeners
══════════════════════════════════════════════════════════ */
'use strict';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pages sans hero : nav solid immédiatement
...
/* ─────────────────────────────────────────────────────────
   1. PAGE TRANSITION
   Fade-in rapide à chaque chargement (20ms RAF)
───────────────────────────────────────────────────────── */
function initPageTransition() {
    // Injecter l'overlay s'il n'existe pas
    ...
    // Fade-out au départ (liens internes uniquement)
...
/* ─────────────────────────────────────────────────────────
   2. CURSEUR CUSTOM
   Dot instantané · Ring avec lag fluide (lerp)
───────────────────────────────────────────────────────── */
function initCursor() {
    ...
    // Position cible vs position interpolée du ring
    let tx = 0, ty = 0; // target (souris)
    let rx = 0, ry = 0; // ring actuel

    // Dot : suit la souris sans délai via CSS transform
    ...
    // État hover : scale via CSS class
    ...
    // Masque quand la souris sort de la fenêtre
...
/* ─────────────────────────────────────────────────────────
   4. HERO SLIDER
   Ken Burns effect sur chaque slide
───────────────────────────────────────────────────────── */
function initHeroSlider() {
    ...
    // Ajouter l'animation Ken Burns en CSS dynamiquement
...
/* ─────────────────────────────────────────────────────────
   5. SCROLL REVEAL
   IntersectionObserver · Stagger sur groupes
───────────────────────────────────────────────────────── */
function initReveal() {
    ...
    // Stagger automatique pour les grilles d'enfants
...
/* ─────────────────────────────────────────────────────────
   6. COMPTEURS ANIMÉS
   easeOutCubic · RAF · triggered par IO
───────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────
   ...
   Mouse repousse légèrement les boutons .btn
   (effet premium · zero layout thrash)
───────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────
   8. PARALLAX LÉGER
   Hero bg · Images about
   Uniquement en RAF · will-change sur les éléments
───────────────────────────────────────────────────────── */
function initParallax() {
    ...
    function applyParallax() {
        ...
        // About image : parallax plus léger
        if (aboutImg) {
    ...
    // Start loop uniquement quand on scroll
...
/* ─────────────────────────────────────────────────────────
   ...
   Les h1 de page-hero se révèlent mot par mot
───────────────────────────────────────────────────────── */
function initTextSplit() {
    const targets = document.querySelectorAll('.page-hero h1, .hero-title');

    targets.forEach(el => {
        // Récupérer les noeuds texte directs + em/strong
        ...
        children.forEach((node, ni) => {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                // em, strong, span : wrapper les mots dedans aussi
...
/* ─────────────────────────────────────────────────────────
   10. HOVER TILT 3D
   Cards services et reviews se penchent légèrement
   Uniquement desktop · GPU only (transform)
───────────────────────────────────────────────────────── */
function initHoverTilt() {
    ...
    tiltables.forEach(el => {
        ...
        el.addEventListener('mousemove', e => {
            ...
            const rx = y * -6; // rotation X (vertical)
            const ry = x * 6; // rotation Y (horizontal)
...
function initContactForm() {
    ...
    form.addEventListener('submit', async e => {
        ...
        // Ajouter @keyframes spin si absent
      ...
      `;
        ...
        // ─── Remplacer ici par votre appel API / Formspree / EmailJS ───
...
/* ─────────────────────────────────────────────────────────
   15. SWITCHER FR / EN (cookie-based, sans rechargement boucle)
───────────────────────────────────────────────────────── */
function initLangSwitcher() {
    const btns = document.querySelectorAll('.lang-btn');

    // Détection cookie existant
